(function(label, definition) {
  ...
  mat3.eq = function(a, b, epsilon) {
    if (typeof(epsilon) === 'number') {
      return (Math.abs(b[0] - a[0]) <= epsilon) && (Math.abs(b[1] - a[1]) <= epsilon) && (Math.abs(b[2] - a[2]) <= epsilon) && (Math.abs(b[3] - a[3]) <= epsilon) && (Math.abs(b[4] - a[4]) <= epsilon) && (Math.abs(b[5] - a[5]) <= epsilon) && (Math.abs(b[6] - a[6]) <= epsilon) && (Math.abs(b[7] - a[7]) <= epsilon) && (Math.abs(b[8] - a[8]) <= epsilon);
    } else {
  ...
  mat3.inv = function(out, m) {
    ...
    if (!det) {
      return mat3.copy(out, mat3.IDENTITY);
    }
    det = 1.0 / det
  ...
  mat3.array.make = function(count, init) {
    ...
    if (typeof(init) === 'number') {
      mat3.array.forEach(array, function(value) {
        value[0] = value[4] = value[8] = init;
      });
