(function(label, definition) {
  ...
  var node_vec4 = null;
  try {
    node_vec4 = node_vec4 || require('./build/Release/vec4');
  } catch (err) {}
  try {
    node_vec4 = node_vec4 || process._linkedBinding('node_vec4');
  } catch (err) {}
  try {
    node_vec4 = node_vec4 || process.binding('node_vec4');
  } catch (err) {}
  ...
  vec4.eq = function(a, b, epsilon) {
    if (typeof(epsilon) === 'number') {
      return (Math.abs(b[0] - a[0]) <= epsilon) && (Math.abs(b[1] - a[1]) <= epsilon) && (Math.abs(b[2] - a[2]) <= epsilon) && (Math.abs(b[3] - a[3]) <= epsilon);
    } else {
