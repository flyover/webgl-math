...
mat4.objectify = function (str, obj)
{
	...
	obj[0] = parseFloat(match[ 1]); obj[4] = parseFloat(match[ 2]); obj[ 8] = parseFloat(match[ 3]); obj[12] = parseFloat(match[ 4]);
	obj[1] = parseFloat(match[ 5]); obj[5] = parseFloat(match[ 6]); obj[ 9] = parseFloat(match[ 7]); obj[13] = parseFloat(match[ 8]);
	obj[2] = parseFloat(match[ 9]); obj[6] = parseFloat(match[10]); obj[10] = parseFloat(match[11]); obj[14] = parseFloat(match[12]);
	obj[3] = parseFloat(match[13]); obj[7] = parseFloat(match[14]); obj[11] = parseFloat(match[15]); obj[15] = parseFloat(match[16]);
	return obj;
}
...
mat4.random_rot = function (m)
{
	...
	var c = Math.cos(a);
	var s = Math.sin(a);
	m[0] = x * x * t + c;     m[1] = y * x * t + z * s; m[ 2] = z * x * t - y * s; m[ 3] = 0.0;
	m[4] = x * y * t - z * s; m[5] = y * y * t + c;     m[ 6] = z * y * t + x * s; m[ 7] = 0.0;
	m[8] = x * z * t + y * s; m[9] = y * z * t - x * s; m[10] = z * z * t + c;     m[11] = 0.0;
...
mat4.from_rot_xyzcs = function (m, x, y, z, c, s)
{
	...
	m[ 4] = x*y*t - z*s; m[ 5] = y*y*t + c;   m[ 6] = z*y*t + x*s;
	m[ 8] = x*z*t + y*s; m[ 9] = y*z*t - x*s; m[10] = z*z*t + c;
	m[15] = 1.0;
	return m;
}
...
{
	m[ 0] = r[0]*s[0]; m[ 1] = r[1]*s[0]; m[ 2] = r[2]*s[0]; m[ 3] = 0.0; // col 0
	m[ 4] = r[3]*s[1]; m[ 5] = r[4]*s[1]; m[ 6] = r[5]*s[1]; m[ 7] = 0.0; // col 1
	m[ 8] = r[6]*s[2]; m[ 9] = r[7]*s[2]; m[10] = r[8]*s[2]; m[11] = 0.0; // col 2
	m[12] = p[0];      m[13] = p[1];      m[14] = p[2];      m[15] = 1.0; // col 3
	return m;
}
...
{
	var a = m[ 0], b = m[ 1], c = m[ 2];//, p = m[ 3]; // col 0
	var d = m[ 4], e = m[ 5], f = m[ 6];//, q = m[ 7]; // col 1
	var g = m[ 8], h = m[ 9], i = m[10];//, r = m[11]; // col 2
	var x = m[12], y = m[13], z = m[14];//, w = m[15]; // col 3
	...
	if (mat4.det(m) < 0.0) { sx = -sx; }

	p[0] = x;
	p[1] = y;
	p[2] = z;

	r[0] = a / sx; r[1] = b / sx; r[2] = c / sx; // col 0
	r[3] = d / sy; r[4] = e / sy; r[5] = f / sy; // col 1
	r[6] = g / sz; r[7] = h / sz; r[8] = i / sz; // col 2
...
mat4.eq = function (a, b, epsilon)
{
	if (typeof(epsilon) === 'number')
	{
		return (Math.abs(b[ 0] - a[ 0]) <= epsilon)
			...
			&& (Math.abs(b[ 9] - a[ 9]) <= epsilon)
			&& (Math.abs(b[10] - a[10]) <= epsilon)
			&& (Math.abs(b[11] - a[11]) <= epsilon)
			&& (Math.abs(b[12] - a[12]) <= epsilon)
			&& (Math.abs(b[13] - a[13]) <= epsilon)
			&& (Math.abs(b[14] - a[14]) <= epsilon)
			&& (Math.abs(b[15] - a[15]) <= epsilon);
	}
	else
	{
		return a[ 0] === b[ 0] 
...
mat4.inv = function (out, m)
{
	...
	if (!det) { return mat4.copy(out, mat4.IDENTITY); }
	det = 1.0 / det;
...
mat4.transform_vec4_array = function (out, m, v, start, count)
{
	...
	for (var index = start; index < count; ++index)
	{
		...
		var v0 = v[offset + 0], v1 = v[offset + 1], v2 = v[offset + 2], v3 = v[offset + 3];
...
mat4.array.make = function (count, init)
{
	...
	if (typeof(init) === 'number')
	{
		mat4.array.forEach(array, function (value) { value[0] = value[5] = value[10] = value[15] = init; });
	}
