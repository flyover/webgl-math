...
{
	NanScope();
	if (!IS_VEC4(args[0])) return NanThrowError("out not a vec4");
	if (!IS_VEC4(args[1])) return NanThrowError("a not a vec4");
	if (!IS_VEC4(args[2])) return NanThrowError("b not a vec4");
...
{
	NanScope();
	if (!IS_VEC4_ARRAY(args[0])) return NanThrowError("out not a vec4.array");
	if (!IS_VEC4_ARRAY(args[1])) return NanThrowError("a not a vec4.array");
	if (!IS_VEC4_ARRAY(args[2])) return NanThrowError("b not a vec4.array");
	if (!(args[3]->IsNumber())) return NanThrowError("s not a number");
	float* out = (float*) args[0].As<v8::Object>()->GetIndexedPropertiesExternalArrayData();
	const float* a = (float*) args[1].As<v8::Object>()->GetIndexedPropertiesExternalArrayData();
	const float* b = (float*) args[2].As<v8::Object>()->GetIndexedPropertiesExternalArrayData();
